        ...
        // if no errors
            ...
            if (this.loaded === true) {
				...
				if (showSomething) {
					...
					leftArrow.addEventListener("click",() => prevImage(this.imageIndex,this.imageList));
					var  rightArrow = document.createElement('button');
					rightArrow.innerText = ">";
					rightArrow.className = "carousel-arrow";
					rightArrow.addEventListener("click",() => nextImage(this.imageIndex,this.imageList));
					...
					/*function updateDots(index){
						...
						dot.style.backgroundColor = index === this.imageIndex ? "red" : "grey";
					});
					}*/
					...
					//updateDots(this.imageIndex);
					// ad the image to the dom
					carouselContainer.appendChild(leftArrow);
					carouselContainer.appendChild(image);
					carouselContainer.appendChild(rightArrow);
					
					function updateImg(index, imgList){
						image.src = encodeURI(imgList[index]);
					};
					function prevImage(index,imgList){
						console.log(index + " before click")
						this.imageIndex = (index - 1 + imgList.length) % imgList.length;
						console.log(this.imageIndex + " after click")
						updateImg(this.imageIndex,imgList);
					};
					function nextImage(index,imgList){
						console.log(index + " before click")
						this.imageIndex = (index + 1) % imgList.length;
						console.log(this.imageIndex + " after click")
						updateImg(this.imageIndex,imgList);
					};
					for (let i = 0; i < this.imageList.length; i++){
					 ...
					 //dot.addEventListener("click",() => console.log('clicked!'));
					 dots.push(dot);	
